...
const PING_PERIOD_THRESHOLD = 60; // in seconds
var timeoutFunction, self;
var timeout = function () {
  debug('Timeout Reached, sending disconnect packet');
...
Parser.prototype.receive = function receive(buffer) {
  ...
  while ((index = this.buffer.indexOf(Parser.END)) > -1) {
    ...
    if (json.length > 0) {
      debug("[TWITTER STREAM OUTPUT]: ", json);
      try {
      ...
      if (start !== -1) {
          ...
          var secondsElapsed = (end[0] * NS_PER_SEC + end[1]) / NS_PER_SEC;
          debug('PING took ' + secondsElapsed + ' seconds');
          clearTimeout(timeoutFunction);
      }
      debug("[RECEIVED PING?] ", (new Date()).toISOString());
      start = process.hrtime();
      timeoutFunction = setTimeout(timeout, (PING_PERIOD_THRESHOLD * 1000));

var	VERSION = '0.2.13',
	...
	FormData = require('./simple-form-data');
...
// Upload resources
Twitter.prototype.upload = function(params, callback) {
	var url = 'https://upload.twitter.com/1.1/media/upload.json';
	var form = new FormData();
	var content_type = 'multipart/form-data; boundary=' + form.getBoundary();
	var options = {};
	for (var param in params)
		options[param] = params[param];
	form.append('media', params.buffer, options);
	content = form.toBuffer();
	this.post(url, content, content_type, callback);
	return this;
};

Twitter.prototype.uploadAndUpdate = function(status, media, callback) {
	var self = this;
	var ps = [];
	if (!Array.isArray(media))
		media = [media];
	for (var i = 0; i < media.length; i++) {
		ps[i] = new Promise(function(resolve, reject) {
			self.upload(media[i], function(data) {
	...
	Promise.all(ps).then(function(values) {
		...
		for (var i = 0; i < values.length; i++)
			ids.push(values[i].media_id_string);
		var params = { media_ids: ids.join(',') };
		self.updateStatus(status, params, callback);
	}, function(reason){
		callback(reason);
	});
	return this;
